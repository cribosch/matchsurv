...
```{r}
haz.data<-data.sim(5000,5)
head(haz.data)
```

The option`competing=TRUE` of `data.sim` functions allows to estimate data in a competing risk setting where the excess risk is defined in terms of cause specific hazard. When `nullmod=TRUE`, the excess risk does not depend on covariates, so no covariates are simulated.  
